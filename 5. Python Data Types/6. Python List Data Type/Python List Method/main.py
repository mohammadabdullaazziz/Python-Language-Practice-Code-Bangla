...
পাইথনে কোনো লিস্টের একদম সামনে (শুরুতে) কোনো সংখ্যা যোগ করার জন্য মূলত দুটি চমৎকার উপায় আছে:

insert() ফাংশন ব্যবহার করে (সবচেয়ে জনপ্রিয়)
insert() মেথডের কাজ হলো নির্দিষ্ট ইনডেক্সে নতুন মান বসানো। যেহেতু আপনি একদম সামনে বসাতে চাচ্ছেন, তাই ইনডেক্স হবে 0।

number = [1, 2, 3, 4, 5]

# ০ নম্বর ইনডেক্সে 0 যোগ করো
number.insert(0, 0)

print(number)
# আউটপুট: [0, 1, 2, 3, 4, 5]

লিস্ট যোগ করে (+ অপারেটর)
সরাসরি দুটি লিস্ট একসাথে যোগ করেও:

number = [1, 2, 3, 4, 5]

# [0] লিস্টের সাথে মূল লিস্ট যোগ করা
number = [0] + number
...
# সরাসরি রিমুভ করা হলো, কোনো ভ্যারিয়েবলে রাখা হলো না
all_files.remove("photo1.jpg")
...
যদি ডিলিট করার পরও লিস্টটি অন্য কোনো ভ্যারিয়েবলে রাখতে :
...
এই কোডের দারুণ সুবিধাগুলো:
১. মূল লিস্ট সুরক্ষিত থাকে (all_files অপরিবর্তিত থাকে):
আগের .remove() মেথড ব্যবহার করলে মূল লিস্ট থেকে ফাইলটি চিরতরে হারিয়ে যেত।
কিন্তু এই পদ্ধতিতে মূল all_files লিস্ট আগের মতো ঠিকই অক্ষত থাকবে (["photo1.jpg", "document.pdf", ...])।
২. kk ভ্যারিয়েবলে নতুন লিস্ট পাওয়া যায়:
kk-এর ভেতরে "photo1.jpg" ছাড়া বাকি সব ফাইলের একটি একদম নতুন লিস্ট তৈরি হয়ে জমা হবে
(['document.pdf', 'document.png', 'photos.jpg', 'notes.png', 'banner.jpg'])। এখানে কোনো None আসার ঝামেলা নেই।
...
এখানে "Red" আর লিস্টে থাকবে না। কারণ .pop(0) মেথডটি মূল লিস্টের 0 নম্বর ইনডেক্সে থাকা "Red"-কে চিরতরে মুছে ফেলেছে।
...
পাইথনে কখন এবং কেন None রিটার্ন করে?
পাইথনে সাধারণত দুই ধরনের ফাংশন বা মেথড দেখা যায়:

১. যেগুলো সরাসরি পরিবর্তন করে (In-place Modifiers) - এরা None রিটার্ন করে
যেসব মেথড কোনো ডেটা স্ট্রাকচারকে সরাসরি মডিফাই করে, তারা নতুন কিছু রিটার্ন করে না। তাই এদের কোনো ভ্যারিয়েবলে রাখলে None পাওয়া যায়।
...
.append() (লিস্টে নতুন আইটেম যোগ করে)

.remove() (লিস্ট থেকে আইটেম মুছে ফেলে)


ভুল উদাহরণ: new_list = nums.sort() এখানে new_list আসলে None হয়ে যাবে।
সঠিক নিয়ম: প্রথমে nums.sort() লিখে লাইন শেষ করতে হবে, তারপর print(nums) করতে হবে।

২. যেগুলো নতুন মান তৈরি করে রিটার্ন করে - এরা কখনো None রিটার্ন করে না (যদি ডেটা থাকে)
যেসব মেথড বা ফাংশন মূল ডেটা হাত দেয় না, বরং হিসাব করে বা ফিল্টার করে নতুন কিছু দেয়, তারা মান রিটার্ন করে।

উদাহরণ:

sorted(nums) (এটি মূল লিস্ট ঠিক রেখে একটি নতুন সর্ট করা লিস্ট রিটার্ন করে। তাই এখানে kk = sorted(nums) লিখলে কাজ করবে!)

len(nums) (সংখ্যা রিটার্ন করে)

স্ট্রং বা ম্যাথ ফাংশনগুলো।
...
for fruit in fruits: এই লুপটি মূল লিস্ট থেকে একেকটি উপাদানকে আলাদা করে ফেলে।

লুপের ভেতরে fruit ভ্যারিয়েবলটি পুরো লিস্টকে ধরে না, এটি প্রতিবার একটি করে ফল (যেমন: প্রথমে "Apple", দ্বিতীয়বার "Banana", এভাবে) ধরে।

তাই print(fruit) দিলে পাইথন শুধু ওই একক ফলটির নাম প্রিন্ট করে, পুরো লিস্টটি বা তার ব্র্যাকেট প্রিন্ট করে না।
...
১. len(fruits) কেন নেওয়া হলো?
len() ফাংশনটি কোনো লিস্টের ভেতরে মোট কয়টি উপাদান (Item) আছে তা গুনে বের করে।

এই fruits লিস্টে মোট ৩টি ফল আছে ("Apple", "Banana", "Mango")।

তাই len(fruits) লিখলে পাইথন সেটিকে গুনে সংখ্যা হিসেবে 3 বানিয়ে নেয়।

২. এর বাইরে range() কেন নেওয়া হলো?
পাইথনকে সরাসরি শুধু 3 দিলে সে বুঝতে পারে না যে কী করতে হবে। তার একটি ক্রম বা সিকোয়েন্স দরকার হয়।

range() ফাংশনটি একটি নম্বর বা ইনডেক্সের সিকোয়েন্স তৈরি করে দেয়।

যখন  range(len(fruits)) অর্থাৎ range(3), তখন পাইথন ০ থেকে শুরু করে ২ পর্যন্ত ইনডেক্স নম্বরগুলো তৈরি করে দিল: 0, 1, 2।

এই নম্বরগুলোই লুপের ভেতর i ভ্যারিয়েবলের মাধ্যমে একে একে পাস হয়।


৩. print(i, fruits[i]) কী বোঝায়?
লুপ চলার সময় এই লাইনটি মূলত ইনডেক্স নম্বর এবং তার বিপরীতে থাকা ফলের নাম প্রিন্ট করে। এখানে দুইটা জিনিস আলাদাভাবে কাজ করে:


i  এটি হলো ইনডেক্স নম্বর (যেমন: 0, 1, বা 2)।
fruits[i]  এটি হলো ওই ইনডেক্স নম্বরে লিস্টের ভেতরে থাকা আসল ফলটি।

যখন i = 0 হয়, তখন fruits[0] প্রিন্ট করে  Apple
যখন i = 1 হয়, তখন fruits[1] প্রিন্ট করে  Banana
যখন i = 2 হয়, তখন fruits[2] প্রিন্ট করে  Mango
...
for i in range(len(fruits)):
  print(i, len(fruits[i]))

১. fruits = ["Apple", "Banana", "Mango"]
এখানে তিনটি ফলের একটি লিস্ট তৈরি করা হয়েছে।

২. range(len(fruits))

প্রথমে len(fruits) অংশটি লিস্টের উপাদান গুনে বের করে যে এখানে মোট ৩টি ফল আছে।

এরপর range(3) ফাংশনটি ০ থেকে শুরু করে ইনডেক্স নম্বর তৈরি করে দেয়, যার মানগুলো হলো: 0, 1, 2।

৩. for i in range(len(fruits)):
লুপটি প্রতিবার ঘুরে i ভ্যারিয়েবলে একে একে ইনডেক্স নম্বরগুলো নেয় (প্রথমে ০, এরপর ১, এরপর ২)।

৪. print(i, len(fruits[i]))

এখানে প্রথম অংশ i প্রিন্ট করে ফলের ইনডেক্স নম্বর।

দ্বিতীয় অংশ len(fruits[i]) ইনডেক্স ধরে নির্দিষ্ট ফলটির নাম বের করে এবং len() দিয়ে তার ভেতরের অক্ষরের সংখ্যা গুনে প্রিন্ট করে।
...
২.  "A" লিস্টে আছে কি না। কিন্তু লিস্টে কোথাও হুবহু একক বা একা একা "A" নামে কোনো আইটেম নেই। (আইটেমগুলো হলো পুরো আস্ত শব্দ—"Apple", "Banana", "Mango")।
...
n > 0 for n in numbers: এটি একটি জেনারেটর এক্সপ্রেশন। এটি লিস্টের প্রতিটি সংখ্যাকে চেক করে দেখছে সংখ্যাটি ০-এর চেয়ে বড় কি না।

-5 > 0 False
-2 > 0 False
0 > 0  False
10 > 0 True (কারণ ১০ শূন্য থেকে বড়)

২. ফলাফলগুলোর একটি লিস্ট বা সিকোয়েন্স তৈরি হয়: [False, False, False, True]
৩. any(...): এই ফাংশনটি চেক করে দেখে এই সিকোয়েন্সে অন্তত একটি হলেও True আছে কি না। 
যেহেতু শেষটি True, তাই any() পুরোটার ফলাফল হিসেবে True প্রিন্ট করে।




কিছু ছাত্রের পরীক্ষার স্কোরের একটি লিস্ট আছে।  তাদের মধ্যে কেউ পাস (>= 40) করেছে কি না।

scores = [25, 30, 38, 45, 20]

# n এর জায়গায় অন্য ভ্যারিয়েবল যেমন score ব্যবহার করা হলো
has_passed = any(score >= 40 for score in scores)

print(has_passed)

পাইথন scores লিস্টের প্রতিটি score-কে ধরে চেক করে সেটি 40 বা তার চেয়ে বেশি কি না:

25 >= 40  False
30 >= 40 False
38 >= 40 False
45 >= 40 True (এখানে শর্ত মিলে গেছে!)
20 >= 40  False

২. পাইথন যখনই দেখল 45 এর ক্ষেত্রে শর্তটি True হয়ে গেছে, any() তখন আর বাকিগুলো চেক না করে সঙ্গে সঙ্গে থেমে গিয়ে ফলাফল হিসেবে True রিটার্ন করে দিল।

৩. ফলে has_passed ভ্যারিয়েবলে True সেভ হলো এবং আউটপুটে True প্রিন্ট হলো।

এভাবে জেনারেটর এক্সপ্রেশনের ভেতরে যেকোনো ভ্যারিয়েবল নাম (n, score, item ইত্যাদি) ব্যবহার করে খুব সহজেই শর্ত যাচাই করা যায়!
...
স্ট্রিংয়ে কোনো সংখ্যা (Digit) আছে কি না চেক করা (any)
ধরে নিন, একটি পাসওয়ার্ডে অন্তত একটি সংখ্যা আছে কি না তা চেক

password = "abcde1fgh"

# জেনারেটর এক্সপ্রেশন দিয়ে চেক করা হচ্ছে কোনো ডিজিট আছে কিনা
has_digit = any(char.isdigit() for char in password)

print(has_digit)  
# আউটপুট: True (কারণ '1' একটি সংখ্যা)




# উদাহরণ ১: একটি স্বাভাবিক স্ট্রিং
text = "hello"
print(any(text))  
# আউটপুট: True 
# (কারণ এখানে অনেকগুলো অক্ষর আছে, অন্তত একটা তো True বটেই)

# উদাহরণ ২: কোনো সংখ্যা বা লেটার আছে কিনা চেক করা
text2 = "   "  # শুধু স্পেস
print(any(text2))  # আউটপুট: True (স্পেসও একটি ক্যারেক্টার)

# উদাহরণ ৩: ব্ল্যাংক বা ফাঁকা স্ট্রিং
empty_text = ""
print(any(empty_text))  
# আউটপুট: False 
# (কারণ ফাঁকা স্ট্রিংয়ে কোনো ক্যারেক্টার বা Truthy মান নেই)




স্ট্রিংয়ে কোনো স্বরবর্ণ (Vowel) আছে কি না চেক করা (any)
একটি শব্দে কোনো ভরবর্ণ (a, e, i, o, u) আছে কি না তা চেক করতে

text = "rhythm"
vowels = "aeiou"

# জেনারেটর এক্সপ্রেশন দিয়ে চেক করা হচ্ছে কোনো ভাওয়েল আছে কিনা
has_vowel = any(char in vowels for char in text)

print(has_vowel)  
# আউটপুট: False (কারণ 'rhythm' শব্দে কোনো a, e, i, o, u নেই)



স্ট্রিংয়ের সব অক্ষর কি ইংরেজি বর্ণমালা (Alphabet) কি না চেক করা (all)
ইনপুট নেওয়া কোনো নাম বা টেক্সটে কোনো সংখ্যা বা স্পেশাল ক্যারেক্টার আছে কি না, তা যাচাই করতে all() এবং .isalpha() মেথড ব্যবহার করা হয়।



name = "Abdullah"

# সব অক্ষর বর্ণমালা কি না চেক করা
is_only_letters = all(char.isalpha() for char in name)

print(is_only_letters)  
# আউটপুট: True (কারণ এখানে কোনো স্পেস বা সংখ্যা নেই)

# যদি স্পেস বা সংখ্যা থাকে:
name2 = "Abdullah 123"
print(all(char.isalpha() for char in name2))  
# আউটপুট: False (স্পেস ও সংখ্যার কারণে)




স্ট্রিংয়ে কোনো স্পেশাল ক্যারেক্টার (Special Character) আছে কি না চেক করা (any)
পাসওয়ার্ড ভ্যালিডেশনের সময় অনেক সময় চেক করতে হয় পাসওয়ার্ডে অন্তত একটি স্পেশাল ক্যারেক্টার (@, #, $, ! ইত্যাদি) আছে কি না।


password = "mypassword123!"
special_chars = "@#$!%^&*()"

# কোনো স্পেশাল ক্যারেক্টার আছে কিনা চেক করা
has_special = any(char in special_chars for char in password)

print(has_special)  
# আউটপুট: True (কারণ শেষের দিকে '!' আছে)



স্ট্রিংয়ের সব অক্ষর বড় হাতের (Uppercase) কি না চেক করা (all)
কোনো কোড বা কুপন (Coupon Code) সব বড় হাতের অক্ষরে লেখা আছে কি না, তা যাচাই করতে এটি কাজে লাগে।


coupon = "SAVE50"

# সব অক্ষর বড় হাতের কি না চেক করা
is_upper = all(char.isupper() for char in coupon if char.isalpha())

print(is_upper)  
# আউটপুট: True (এখানে শুধু অক্ষরগুলোর ওপর চেক করা হয়েছে, সংখ্যা বাদ দিয়ে)
