...
পাইথনে স্লাইসিং করার সময় কোলনের পরের ইনডেক্সটি আগের ঘরে গিয়ে থেমে যায় (Stop Index is Exclusive)। 
অর্থাৎ, [0:5] লেখার মানে হলো: ০ নম্বর ইনডেক্স থেকে শুরু হয়ে ৪ নম্বর ইনডেক্স পর্যন্ত চলবে (৫-এর ঠিক আগে থেমে যাবে)।

স্লাইসিংয়ের ৩টি শর্টকাট টিপস
১. শুরু না দিলে: text[:5] লিখলে পাইথন স্বয়ংক্রিয়ভাবে ০ নম্বর (শুরু) থেকে ধরা শুরু করে।
২. শেষ না দিলে: text[6:] লিখলে পাইথন ৬ নম্বর ইনডেক্স থেকে একদম শেষ পর্যন্ত কেটে নিয়ে আসবে (আউটপুট হবে: "Python")।
...
Start = 6: পাইথন ৬ নম্বর ইনডেক্সে যাবে (যেখানে P অক্ষরটি আছে)।Stop = ফাঁকা: কোলনের পরে কিছু লিখেননি মানে পাইথনকে বলা হসছে"একদম শেষ পর্যন্ত কেটে নিয়ে যাও!"আউটপুট: 
...
print(substring_python)

: (একটি কোলন) দিয়ে এখানে কী ঘটল?

কোলনের বাম পাশে 6 দেওয়া হয়েছে এবং ডান পাশ খালি রাখা হয়েছে।

বাম পাশ (start = 6): পাইথন ৬ নম্বর ইনডেক্স থেকে কাটা শুরু করবে। ৬ নম্বর ইনডেক্সে রয়েছে 'P' অক্ষরটি।

ডান পাশ (stop = খালি): কোলনের পর কিছু না লেখার অর্থ হলো "একদম শেষ পর্যন্ত যাও"।

তাই পাইথন ৬ নম্বর ইনডেক্স ('P') থেকে শুরু করে স্ট্রিংয়ের শেষ অক্ষর ('n') পর্যন্ত পুরো অংশটি নিয়ে নিয়েছে।
...
start: কত নম্বর ইনডেক্স থেকে শুরু হবে।

stop: কত নম্বর ইনডেক্সে গিয়ে থামবে (এই ইনডেক্সটি কিন্তু বাদ পড়ে, এর ঠিক আগের ঘরে থামে)।

step: কয় ঘর করে লাফ দেবে (বা কোন দিকে যাবে)।

slice_text = text[ :  : -1]
                 │  │   └─► step  = -1 (উল্টো দিক থেকে ১ ঘর করে পেছনে যাবে)
                 │  └─────► stop  = খালি (একদম শেষ/শুরু পর্যন্ত যাবে)
                 └────────► start = খালি (ডিফল্টভাবে একদম শেষ অক্ষর থেকে শুরু করবে)


যখন start এবং stop খালি রেখে শুধু step এ -1 বসিয়ে দেওয়া হয়, তখন পাইথন বুঝে নেয়: "পুরো স্ট্রিংটির শেষ অক্ষর থেকে শুরু করে উল্টো দিক থেকে এক ঘর এক ঘর করে প্রথম অক্ষর পর্যন্ত আসতে হবে।"


text = "Hello Python"

# ১. দুই ঘর করে সামনে লাফ দেবে
print(text[::2])   # Output: HloPto

# ২. উল্টো দিক থেকে দুই ঘর করে লাফ দেবে
print(text[::-2])  # Output: nhy ol

# ৩. ৩ নম্বর ইনডেক্স থেকে শুরু করে উল্টো দিকে যাবে
print(text[5::-1]) # Output:  olleH

এক কথায়: পাইথনে কোনো স্ট্রিং বা লিস্টকে খুব সহজে এবং এক লাইনে উল্টাতে (Reverse করতে) [::-1] ব্যবহার করা হয়।
...
# ২ নম্বর ইনডেক্স ('n') থেকে শুরু হয়ে ৬ নম্বরের ('d') ঠিক আগে থামবে
result = text[2:6:1]

print(result) # Output: ngla

ব্যাখ্যা: এখানে step = 1, তাই ১ ঘর করে স্বাভাবিকভাবে সামনে গেছে।
...
# ০ থেকে শুরু হয়ে ৯ এর আগে থামবে এবং ২ ঘর করে লাফ দেবে
result = text[0:9:2]

print(result) # Output: 02468

ব্যাখ্যা: 0 নিলো ➔ ১ ঘর লাফ দিয়ে 2 নিলো ➔ আবার লাফ দিয়ে 4 ➔ এভাবে জোড় সংখ্যাগুলো বেছে নিলো।



text = "Python"

# ৫ নম্বর ইনডেক্স ('n') থেকে শুরু করে ১ নম্বরের ('y') ঠিক আগে থেমে যাবে
# step = -1 দেওয়াতে ডান দিক থেকে বামে (উল্টো দিকে) আসবে
result = text[5:1:-1]

print(result) # Output: noht

ব্যাখ্যা: এখানে উল্টো যাত্রায় 'P' (০ নম্বর ইনডেক্স) নেওয়া হয়নি, কারণ আমরা স্টপ দিয়েছি 1 এ (যা 'y')।
...
# ১. start খালি (ডিফল্ট ০ থেকে শুরু)
print(text[:6:1])    # Output: Progra

# ২. stop খালি (একদম শেষ পর্যন্ত যাবে)
print(text[3::1])    # Output: gramming

# ৩. step খালি (ডিফল্ট ১ ঘর করে যাবে)
print(text[0:4])     # Output: Prog


ধনাত্বক step (+1, +2): বাম থেকে ডানে যায় -- ঋণাত্বক step (-1, -2): ডান থেকে বামে (উল্টো দিকে) আসে
...
# মেম্বারশিপ (in / not in)ঃ

এটি পাইথনের অন্যতম একটি ফিচার। একে বলা হয় মেম্বারশিপ অপারেটর (Membership Operators)।

পাইথনে in এবং not in মেম্বারশিপ অপারেটর (Membership Operators) মূলত কোনো উপাদান (element) নির্দিষ্ট একটি সিক্যুয়েন্স বা ডাটা স্ট্রাকচারে আছে কি নেই তা পরীক্ষা করার জন্য ব্যবহার করা হয়। এটি সরাসরি True বা False আউটপুট দেয়।

এদের মূল কাজ হলো—কোনো একটি ডাটা (অক্ষর, সংখ্যা বা শব্দ) অন্য একটি ডাটা কালেকশনের (যেমন: স্ট্রিং, লিস্ট, ডিকশনারি) ভেতরে সদস্য (Member) হিসেবে আছে কি নেই, তা চেক করা।

এই অপারেটরগুলো সবসময় উত্তর হিসেবে True (সত্য) অথবা False (মিথ্যা) দেয়।

১. in (ভিতরে আছে কি?)

যদি নির্দিষ্ট উপাদানটি কালেকশনের ভেতরে থাকে, তবে এটি True দেবে। না থাকলে False দেবে।

name = "Mohammad"

# 'm' কি name এর মধ্যে আছে?
print("m" in name)   # Output: True

# 'z' কি name এর মধ্যে আছে?
print("z" in name)   # Output: False


২. not in (ভিতরে নেই তো?

এটি in-এর ঠিক উল্টো! উপাদানটি কালেকশনের ভেতরে না থাকলে এটি True দেবে। আর থাকলে False দেবে।

fruits = ["apple", "banana", "mango"]

# 'orange' কি ফলগুলোর তালিকায় নেই?
print("orange" not in fruits)  # Output: True (কারণ কমলা আসলেই তালিকায় নেই)

# 'apple' কি ফলগুলোর তালিকায় নেই?
print("apple" not in fruits)   # Output: False (কারণ আপেল তালিকায় আছে)

in  "এটা কি এর ভেতরে আছে?" (থাকলে True)  not in "এটা কি এর ভেতরে নেই?" (না থাকলে True)
...
ব্যবহারকারী বড় হাতের "Man" লিখুক বা ছোট হাতের "man"—কোড যেন সঠিক পজিশনটা খুঁজে বের করে, তবে দুটোকেই ছোট হাতের বানিয়ে (Lower) সার্চ করতে :

text = "Hello Arman"
search_term = "Man"

# দুটোকেই lower-case বানিয়ে খোজা হচ্ছে
index_pos = text.lower().find(search_term.lower())
...
is_integer() মেথডটি শুধু float ডেটা টাইপের ওপরই কাজ করে, কিন্তু এটি চেক করে যে ফ্লট সংখ্যাটির দশমিকের পর কোনো মান আছে কি না (যেমন .0 থাকলে তা পূর্ণসংখ্যা হিসেবে গণ্য হয়)।


১. সরাসরি ইন্টিজার (int) চেক করার জন্য: isinstance()
পাইথনে সরাসরি কোনো ভ্যারিয়েবল ইন্টিজার (int) কি না, তা চেক করার জন্য isinstance() ফাংশন ব্যবহার করা হয়।


x = 25
y = 10.5

# এটি চেক করবে x কি একটি int কি না
print(isinstance(x, int))  # আউটপুট: True
print(isinstance(y, int))  # আউটপুট: False
